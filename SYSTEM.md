...
**当前状态（2026-07-31 均衡后）**：单一账户 **8/8 满槽**，每标的成本基一律 **$10,000**（C7 校验强制；偏离须显式 SIZE_EXCEPTION）；总资产 **97,127.42**、累计盈亏 **−2,872.58（−2.87%）**、仓位率 **79.23%**（用户授权的均衡计划覆盖 DEFENSIVE 50% 上限）。满槽下新机会只能通过替换进入（候选高于最弱持仓 ≥5 分且旧仓满足退出规则）。
...
- **推送格式（2026-07-27 用户裁定重写，强制）——推送是交易便签，不是系统报告**：
  - 模板（2026-07-31 用户裁定升级：**推送必须让用户看见钱**——成本/当日/累计盈亏为必填）：
    ```
    # 阶段｜日期
    <SYM> · <动作> · 成本<均价>｜今日<±x%>｜累计<±y%> · <一句原因>   ← 每仓一行；盘前用「昨日」
    成交：<trade_id 摘要>                                            ← 仅当本次落账交易
    📊 <SYM> 财报<日期>：共识<EPS/营收>｜<情景计划一句>              ← 持仓财报 ≤5 交易日时
    🚨 <一句风险>                                                    ← 仅当有需要用户知道的风险
    账户：总资产 <X>（今日 <±a%>｜累计 <±b%>）｜现金 <Y>             ← 必填收尾行
    ```
  ...
  > 机器可验证子集（L1/L2/L3/L4/L6/**L7 账务透明**）由 `lib/report_lint.py` 硬门执行，`enqueue()` 默认 lint=True，
